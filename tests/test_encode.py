...
def test_asymmetric():
    image = PIL.Image.open(os.path.join(base_path, "cool_cat.jpg"))
    image_array = np.array(image.convert("RGB"), dtype=np.float)
    blur_hash = encode(image_array, x_components = 2, y_components = 8)
    ...
    blur_hash = encode(image_array, x_components = 8, y_components = 2)
    decoded_image = decode(blur_hash, 32, 32)
    assert np.sum(np.var(decoded_image, axis = 0)) < np.sum(np.var(decoded_image, axis = 1))
...
def test_components():
    image = PIL.Image.open(os.path.join(base_path, "cool_cat.jpg"))
    image_array = np.array(image.convert("RGB"), dtype=np.float)
    blur_hash = encode(image_array, x_components = 8, y_components = 3)
...
def test_linear_dc_only():
    ...
    blur_hash = encode(linearish_image, x_components = 1, y_components = 1, linear = True)
...
def test_invalid_parameters():
    ...
    with pytest.raises(ValueError):
        encode(image, x_components = 0, y_components = 1)
    
    with pytest.raises(ValueError):
        encode(image, x_components = 1, y_components = 0)    
    
    with pytest.raises(ValueError):
        encode(image, x_components = 1, y_components = 10)
      
    with pytest.raises(ValueError):
        encode(image, x_components = 10, y_components = 1)